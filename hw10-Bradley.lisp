...
Names of ALL group members: Bradley Fargo, Ayeon Lee, Reef Loretto
...
(defunc prod-t (l acc)
  ...
  (cond ((endp l) acc)
        (t (prod-t (rest l) (* acc (first l))))))

; Give at least 4 check= checks for your prod-t function.
(check= (prod-t nil 0) 0)
(check= (prod-t nil 1) 1)
(check= (prod-t nil -2) -2)
(check= (prod-t nil 900) 900)

(check= (prod-t '(3 4) 0) 0)
(check= (prod-t '(3 4) 1) 12)
(check= (prod-t '(3 4) -2) -24)
(check= (prod-t '(3 4) 900) (* 12 900))

(test? (implies (lorp x) (equal (prod-t x 1) (prod x))))
...
(test? (implies (lorp x) (equal (prod* x) (prod x))))
...
Tprod-t : (and (lorp l) (rationalp acc)) => (prod-t l acc) = (* (prod l) acc)

OR could be Tprod-t-useless:
(and (lorp l) (rationalp acc) (equal acc 1)) => (prod-t l acc) = (prod l)
...
(prod l) = (prod-t l 1)
= {def prod-t}
(prod l) =  (cond ((endp l) acc)
                  (t (prod-t (rest l) (* acc (first l)))))
                  
{we need a proof by cases}
Cases:
A) (lorp l) ^ (endp l)
B) (lorp l) ^ (not (endp l))


Huh so in case A I didn't use our lemma. Did I mess up my logic?
Edit: I used it in case B, should be fine.

Case A:
C2a. (endp l)
C3a. (rationalp acc) {prod-t input-contract}
----
(prod l) = (prod-t l 1)
= {def prod-t|((acc 1)),if axiom, C2a, C3a}
(prod l) = 1
= {def prod, C1, C2a}
1 = 1
= {arithmetic}
...
Case B:
C2b. (not (endp l))
C3b. (rationalp acc) {prod-t input-contract}
----
(prod l) = (prod-t l 1)
= {def prod-t|((acc 1)), C2b}
(prod l) = (prod-t (rest l) (* 1 (first l)))
= {arithmetic}
(prod l) = (prod-t (rest l) (first l))
= {def prod, C2b}
(* (first l) (prod (rest l))) = (prod-t (rest l) (first l))
= {Tprod-t|((l (rest l)) (acc (first l)))}
(* (first l) (prod (rest l))) = (* (prod (rest l)) (first l))
= { Commutative }
true
...
Induction Scheme from prod-t:
A) ~[ (lorp l) /\ (rationalp acc) ] => Tprod-t
B) (lorp l) /\ (rationalp acc) /\ (endp l) => Tprod-t
C) (lorp l) /\ (rationalp acc) /\ (not (endp l))
            /\ Tprod-t|((l (rest l)) (acc (* acc (first l)))) => Trod-t
Case A:
A1. ~[ (lorp l) /\ (rationalp acc) ]
A2. ~ (lorp l) \/ ~ (rationalp acc)

(and (lorp l) (rationalp acc)) => (prod-t l acc) = (* (prod l) acc)
= {A1, PL (F => Z = true)}
true
...
(prod-t l acc) = [ (prod l) * acc ]
= {def prod-t, def prod, B3}
acc = 1 * acc
= {arithmetic}
acc = acc
= {reflexivity}
...
C2. (rationalp acc)
C3. (not (endp l))
C4. Tprod-t|((l (rest l)) (acc (* acc (first l))))
C5. (lorp (rest l)) /\ (rationalp [acc * (first l)])
           => (prod-t (rest l) [acc * (first l)] = (prod (rest l)) * [acc * (first l)]
------------
C6. (lorp (rest l)) {def lorp, C1}
C7. (lorp (first l)) {def lorp, C1}
C8. (rationalp [acc * (first l)]) {def rationalp, C7, C2}
C9. (prod-t (rest l) [acc * (first l)] = (prod (rest l)) * [acc * (first l)]
           {C5, C8, C6}
